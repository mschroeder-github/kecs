...
public class AuthController extends AbstractTokenController {
    ...
    public static final String BCRYPT_SALT = "$2a$10$MOu3S18JDjMntZRv/hum0."; //BCrypt.gensalt();
    ...
    private String login(Request request, Response response) throws IOException {
        ...
        if (validatePost(jsonRequest)) {
            try {
                ...
                interaction.put("uri", AUTH_ENDPOINT_PREFIX + "/login");
    ...
    */
    ...
    private void createAdminUser() {
        if(!userService.exists("admin")) {
            userService.register("admin", userService.getSettings().getAdminPassword(), BCrypt.hashpw(userService.getSettings().getAdminPassword(), BCRYPT_SALT), "LOGD", "Admin", userService.getSettings().getRegisterSecret());
